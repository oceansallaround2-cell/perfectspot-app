...
import { Star, Trash2, Loader2, Search, Plus, BookHeart } from "lucide-react";
...
function JournalPage() {
  ...
  async function write(e: React.FormEvent) {
    ...
    else { setContent(""); toast.success("Saved 💜"); }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={write} className="glass-card space-y-3 p-5">
        ...
        />
        <Button type="submit" disabled={saving || !content.trim()} className="btn-romantic w-full rounded-full">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded-full border border-border/60 px-4 py-2 text-xs font-medium"
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] uppercase tracking-widest text-muted-foreground">{new Date(e.created_at).toLocaleString()}</div>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => toggleFav(e)} className="rounded-full p-2 text-muted-foreground transition hover:text-primary">
                    <Star className="h-4 w-4" fill={e.is_favorite ? "currentColor" : "none"} style={e.is_favorite ? { color: "var(--primary)" } : {}} />
                  </button>
                  {e.author_id === user.id && (
                    <button onClick={() => remove(e.id)} className="rounded-full p-2 text-muted-foreground opacity-0 transition group-hover:opacity-100 hover:bg-destructive/10 hover:text-destructive">
